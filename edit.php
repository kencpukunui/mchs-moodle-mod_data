...
if ($datarecord = data_submitted() and confirm_sesskey()) {
    if ($rid) {
        ...
        if ($processeddata->validated && $recordid = data_add_record($data, $currentgroup)) {
            ...
            /*// Priya's old code only trigger when 2 checkbox has been ticked
                if (strpos($emaillist, "##")) {
            ...
            */
            //27062018 (BUG FIX)Let Admin user can receive email after they ticked one/multiple checkbox
            ...
            $sql = "SELECT strip_tags2(dc.content) as text
                    FROM {data_content} dc
                    JOIN {data_fields} df
                    ON df.id = dc.fieldid
                    JOIN {data_records} dr
            ...
            $a = new stdClass();
            $a->jobid = $rec->jobid;
            //26062018 (BUG FIX)change the input parameter to get the specific field from $requestor,$title,$desc
