 ...
 * 15062018 Change priya's code
 * This code will change two table's records in the database (mdl_data_records and mdl_data_content)
 * If the Admin's user / Teacher has changed db job status from New to Completed, after run scheduled tasks of Archived Completed DB jobs
 * It will move the record from Maintenance to Archived without add one more new record to Archived database.
 * This code will change datail from 6->7 in the table mdl_data_records and also change fieldid from 31->40, 32->41, 33->42 ....(9 fields) in the table mdl_data_content 
 */
function mod_data_completedjobs() {
   global $DB;

   $archid = $DB->get_field('data', 'id', array('name' => 'Archived')); // 7
   $maintainid = $dbid = $DB->get_field('data', 'id', array('name' => 'Maintenance')); // 6

   //retuen 2 fields
   $oldsql = "SELECT dc.recordid as fieldrecordid
              FROM {data_content} dc
              JOIN {data_fields} df
              ON df.id = dc.fieldid
              WHERE dc.content = 'Completed'
              AND df.name = 'Status'";
   $getrecordids = $DB->get_records_sql($oldsql);
   
   //do twice
   foreach($getrecordids as $getrecordid){
       
      $sql = "UPDATE {data_records} dr
              SET dr.dataid = :archid 
              WHERE dr.id = :recordid
              AND dr.dataid = :maintainid";
      $DB->execute($sql, array('archid' => $archid,'recordid' => $getrecordid->fieldrecordid, 'maintainid' => $maintainid));
      
      //Date Requested 
      $sql = "UPDATE {data_content} dc
              SET dc.fieldid = (SELECT df.id
                                FROM {data_fields} df
                                WHERE df.dataid = :archid
                                AND df.name = 'Date Requested')
              WHERE dc.recordid = :recordid
              AND dc.fieldid = (SELECT df1.id
                                FROM {data_fields} df1
                                WHERE df1.dataid = :maintainid
                                AND df1.name = 'Date Requested')";
      $DB->execute($sql, array('archid' => $archid, 'recordid' => $getrecordid->fieldrecordid, 'maintainid' => $maintainid));
      ...
      $sql = "UPDATE {data_content} dc
              SET dc.fieldid = (SELECT df.id
                                FROM {data_fields} df
                                WHERE df.dataid = :archid
                                AND df.name = 'Description')
              WHERE dc.recordid = :recordid
              AND dc.fieldid = (SELECT df1.id
                                FROM {data_fields} df1
                                WHERE df1.dataid = :maintainid
                                AND df1.name = 'Description')";
      $DB->execute($sql, array('archid' => $archid, 'recordid' => $getrecordid->fieldrecordid, 'maintainid' => $maintainid));
      ...
      $sql = "UPDATE {data_content} dc
              SET dc.fieldid = (SELECT df.id
                                FROM {data_fields} df
                                WHERE df.dataid = :archid
                                AND df.name = 'Deadline Date')
              WHERE dc.recordid = :recordid
              AND dc.fieldid = (SELECT df1.id
                                FROM {data_fields} df1
                                WHERE df1.dataid = :maintainid
                                AND df1.name = 'Deadline Date')";
      $DB->execute($sql, array('archid' => $archid, 'recordid' => $getrecordid->fieldrecordid, 'maintainid' => $maintainid));
      ...
      $sql = "UPDATE {data_content} dc
              SET dc.fieldid = (SELECT df.id
                                FROM {data_fields} df
                                WHERE df.dataid = :archid
                                AND df.name = 'Assigned to')
              WHERE dc.recordid = :recordid
              AND dc.fieldid = (SELECT df1.id
                                FROM {data_fields} df1
                                WHERE df1.dataid = :maintainid
                                AND df1.name = 'Assigned to')";
      $DB->execute($sql, array('archid' => $archid, 'recordid' => $getrecordid->fieldrecordid, 'maintainid' => $maintainid));
      ...
      $sql = "UPDATE {data_content} dc
              SET dc.fieldid = (SELECT df.id
                                FROM {data_fields} df
                                WHERE df.dataid = :archid
                                AND df.name = 'Name of Requestor')
              WHERE dc.recordid = :recordid
              AND dc.fieldid = (SELECT df1.id
                                FROM {data_fields} df1
                                WHERE df1.dataid = :maintainid
                                AND df1.name = 'Name of Requestor')";
      $DB->execute($sql, array('archid' => $archid, 'recordid' => $getrecordid->fieldrecordid, 'maintainid' => $maintainid));
      ...
      $sql = "UPDATE {data_content} dc
              SET dc.fieldid = (SELECT df.id
                                FROM {data_fields} df
                                WHERE df.dataid = :archid
                                AND df.name = 'Room Number')
              WHERE dc.recordid = :recordid
              AND dc.fieldid = (SELECT df1.id
                                FROM {data_fields} df1
                                WHERE df1.dataid = :maintainid
                                AND df1.name = 'Room Number')";
      $DB->execute($sql, array('archid' => $archid, 'recordid' => $getrecordid->fieldrecordid, 'maintainid' => $maintainid));
      ...
      $sql = "UPDATE {data_content} dc
              SET dc.fieldid = (SELECT df.id
                                FROM {data_fields} df
                                WHERE df.dataid = :archid
                                AND df.name = 'Pending Vendor')
              WHERE dc.recordid = :recordid
              AND dc.fieldid = (SELECT df1.id
                                FROM {data_fields} df1
                                WHERE df1.dataid = :maintainid
                                AND df1.name = 'Pending Vendor')";
      $DB->execute($sql, array('archid' => $archid, 'recordid' => $getrecordid->fieldrecordid, 'maintainid' => $maintainid));
   }

   
   /*  $dbid = $DB->get_field('data', 'id', array('name' => 'Archived')); // 7
 //  echo('dbid : </br>----------------</br>');
 //  var_dump($dbid);
   $arcfields = $DB->get_records('data_fields', array('dataid' => $dbid));
//   echo('arcfields : </br>----------------</br>');
//   var_dump($arcfields); 
   
   $arcfieldarr = array();
   foreach ($arcfields as $arc) {
      $arcfieldarr[$arc->name] = $arc->id;
      echo('arcfieldarr : </br>----------------</br>');
      var_dump($arcfieldarr);
   }

  
   // Get the record that need to be updated.
   $sql = "SELECT dc.recordid, dc.fieldid, df.name
           FROM {data_content} dc
           JOIN {data_fields} df
           ON df.id = dc.fieldid
           WHERE dc.content = 'Completed'
           AND df.name = 'Status'";
   $datarecords = $DB->get_records_sql($sql);
   //echo('datarecords : </br>----------------</br>');
   //var_dump($datarecords);
   foreach ($datarecords as $rd) {
       $rec = new stdClass();
       $rec->id = $rd->recordid;
       $rec->dataid = $dbid;
       //$DB->update_record('data_records', $rec);
       $sql = "SELECT dc.id, dc.recordid, dc.fieldid, df.name
               FROM {data_content} dc
               JOIN {data_fields} df
               ON df.id = dc.fieldid
               WHERE dc.recordid = $rd->recordid";
       $datacontent = $DB->get_records_sql($sql);
       echo('datacontent : </br>----------------</br>');
        var_dump($datacontent);
       foreach ($datacontent as $dc) {
          $sql = "UPDATE {data_content}
                 SET fieldid = :fieldid
                 WHERE recordid = :recordid
                 AND fieldid = :oldfieldid";
           $DB->execute($sql, array('fieldid' => $arcfieldarr[$dc->name], 'recordid' => $dc->recordid, 'oldfieldid' => $dc->fieldid));
       }
   }*/
